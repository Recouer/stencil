...
do
    ...
    for (( i=1; i <= $ITER; i++ )); do
      salloc --exclusive -N2 -n4 mpirun --map-by ppr:2:node ./stencil_MPI_Pure >> $CSV_DIR/$CSV_FILENAME
    done
done
...
do
    ...
    for (( i=1; i <= $ITER; i++ )); do
      salloc --exclusive -N2 -n4 -c6 mpirun --map-by ppr:2:node ./stencil_MPI_omp >> $CSV_DIR/$CSV_FILENAME
    done
done
