...
do
    ...
    for (( i=1; i <= $ITER; i++ )); do
      salloc -proutage --exclusive -N4 -n16 mpirun --map-by ppr:4:node ./stencil_MPI_Pure >> $CSV_DIR/$CSV_FILENAME
    done
done
...
do
    ...
    for (( i=1; i <= $ITER; i++ )); do
      salloc -proutage --exclusive -N4 -n16 -c4 mpirun --map-by ppr:4:node ./stencil_MPI_omp >> $CSV_DIR/$CSV_FILENAME
    done
done
